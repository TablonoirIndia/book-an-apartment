...
const MessageBubble = ({ message, timeStr, isNew }) => {
  const isUser = message.sender === "user";
  
  return (
    <div className={`message-bubble ${isUser ? 'message-bubble-user' : 'message-bubble-admin'}`}>
      {!isUser && <div className="message-avatar">🏠</div>}
      <div className="message-content">
        <div className="message-bubble-text">
          {message.message}
        </div>
        <div className={`message-meta ${isUser ? 'message-meta-user' : 'message-meta-admin'}`}>
          <span className="message-time">{timeStr(message.created_at)}</span>
          {isUser && <span className="message-tick">✓✓</span>}
        </div>
      </div>
    </div>
  );
...
export default function ChatBubble() {
  ...
  return (
    <>
      {/* Chat Panel */}
      {isOpen && (
        <div className={`chat-container ${animOut ? 'chat-container-closing' : 'chat-container-open'}`}>
          {/* Header */}
          <div className="chat-header">
            ...
            <div className="chat-info">
              ...
              <div className={`chat-status ${isAdminTyping ? 'chat-status-typing' : 'chat-status-online'}`}>
                {isAdminTyping ? (
                  <span className="chat-typing-indicator">
                    <span className="chat-typing-text">Support is typing</span>
                    <span className="chat-typing-dots">
                      {[0, 1, 2].map(i => (
                        <span key={i} className="chat-typing-dot" />
                      ))}
                    </span>
                  </span>
                ) : (
          ...
          <div className="chat-messages">
            ...
            {!loading && messages.length === 0 && (
              <div className="chat-empty">
                ...
                <div className="chat-empty-title">Hi {user?.name?.split(" ")[0]}!</div>
            ...
            {messages.reduce((acc, msg, idx) => {
              const date = msg.created_at
                ...
                : null;
              const prevDate = idx > 0 && messages[idx - 1].created_at
                ? new Date(messages[idx - 1].created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })
                : null;
          ...
          <div className="chat-input-area">
            <textarea
              ref={inputRef}
              className={`chat-textarea ${inputFocused ? 'chat-textarea-focused' : ''}`}
              value={input}
              onChange={handleInputChange}
              onKeyDown={handleKeyDown}
              onFocus={() => setInputFocused(true)}
            ...
            <button
              onClick={handleSend}
              disabled={sending || !input.trim()}
              className={`chat-send-btn ${sending || !input.trim() ? 'chat-send-btn-disabled' : 'chat-send-btn-active'}`}
            >
              {sending ? "⏳" : "➤"}
            </button>
          </div>
      ...
      <button
        ...
        className={`chat-floating-btn 
          ${isOpen ? 'chat-floating-btn-open' : 'chat-floating-btn-closed'} 
          ${bubblePop ? 'chat-floating-btn-pulse' : ''} 
          ${unread > 0 && !isOpen ? 'chat-floating-btn-pulse-animation' : ''}`}
      >
        {!isOpen && <span className="chat-floating-pulse-ring" />}
        <span className={`chat-floating-icon ${isOpen ? 'chat-floating-icon-rotated' : 'chat-floating-icon-normal'}`}>
          {isOpen ? "×" : "💬"}
        </span>
        {!isOpen && unread > 0 && (
