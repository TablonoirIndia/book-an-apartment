...
const FloorPlanPage = () => {
  ...
  useEffect(() => {
    if (!floorId) {
      setFetchError("No block ID in URL.");
      setIsFetching(false);
      return;
    }
    ...
    const fetchAll = async () => {
      try {
        const blockRes = await fetch(`${API_BASE}/block/${floorId}`, {
          headers: { Accept: "application/json" }
        });
        
        if (!blockRes.ok) {
          throw new Error(`Block API error ${blockRes.status}`);
        }
        
        const blockData = await blockRes.json();
        
        if (!blockData.panorama_url) {
          throw new Error("No panorama uploaded for this floor yet.");
        }
        ...
        const hotspotRes = await fetch(`${API_BASE}/hotspots?block_id=${floorId}`, {
          headers: { Accept: "application/json" }
        });
        ...
        if (needsStatus) {
          enriched = await Promise.all(
            rawHotspots.map(async (h) => {
              ...
              try {
                const uRes = await fetch(`${API_BASE}/unit/${h.plot_id}`, {
                  headers: { Accept: "application/json" }
                });
                const uData = uRes.ok ? await uRes.json() : {};
                return { ...h, status: uData.status || "unknown" };
    ...
    const initViewer = async () => {
      ...
      markersPluginRef.current.addEventListener("select-marker", ({ marker }) => {
        ...
        if (isAvailable(status)) {
          ...
          hotspots.forEach((h) => {
            try {
              markersPluginRef.current?.updateMarker({ id: `unit-${h.id}`, opacity: 0 });
            } catch (e) {}
          });
          
          if (labelRef.current) {
            labelRef.current.textContent = name;
            labelRef.current.classList.add("floor-unit-label-visible");
          }
          ...
          viewerRef.current
            ...
            .then(() => {
              if (overlayRef.current) {
                overlayRef.current.classList.add("floor-transition-active");
              }
              setTimeout(() => navigate(`/unit/${plotId}`), 450);
            });
        } else {
  ...
  const handleViewUnitDetails = (plotId) => {
    setPopup(null);
    navigate(`/unit/${plotId}`);
  };

  // Fetching state
  if (isFetching) {
    return (
      <div className="floor-fetching">
        <div className="floor-fetching-card">
          <div className="floor-fetching-icon">🏠</div>
          <div className="floor-fetching-text">Loading floor data...</div>
          <div className="floor-fetching-bar">
            <div className="floor-fetching-bar-fill"></div>
          </div>
        </div>
      </div>
    );
  ...
  if (fetchError || !panoramaUrl) {
    return (
      <div className="floor-error">
        <div className="floor-error-card">
          <div className="floor-error-icon">⚠️</div>
          <h2 className="floor-error-title">Floor Unavailable</h2>
          <p className="floor-error-message">
            {fetchError || "Panorama not uploaded yet for this floor."}
          </p>
          <button onClick={() => navigate(-1)} className="floor-error-btn">
            ← Go Back
          </button>
        </div>
      </div>
  ...
  return (
    <div className="floor-plan-page">
      {/* Header */}
      <div className={`floor-header ${showHeader ? 'floor-header-visible' : 'floor-header-hidden'}`}>
        <button onClick={handleBackClick} className="floor-back-btn">
          ← Back to Building
        </button>
      ...
      {/* Panorama Container */}
      <div ref={containerRef} className="floor-panorama" />

      {/* Loading Overlay */}
      <div className={`floor-loader ${isLoaded ? 'floor-loader-hidden' : ''}`}>
        <div className="floor-loader-card">
          <div className="floor-loader-icon">🏠</div>
          <div className="floor-loader-percent">{loadProgress}%</div>
          <div className="floor-loader-bar">
            <div 
              className="floor-loader-bar-fill" 
              style={{ width: `${loadProgress}%` }}
            />
          </div>
          <div className="floor-loader-text">
            {loadProgress < 30 && "Initializing..."}
            {loadProgress >= 30 && loadProgress < 60 && "Loading floor plan..."}
      ...
      {/* Unit Label (on zoom) */}
      <div ref={labelRef} className="floor-unit-label" />

      {/* Status Popup (non-available units) */}
      {popup && (
        <div className="status-popup-overlay" onClick={() => setPopup(null)}>
          <div className="status-popup" onClick={e => e.stopPropagation()}>
            <button onClick={() => setPopup(null)} className="status-popup-close">
              ×
            </button>

            <div className="status-popup-label">Unit</div>
            <div className="status-popup-unit">{popup.label}</div>

            {(() => {
              const cfg = getStatus(popup.status);
              const message = cfg.label === "Pre-Booked" 
                ? "This unit has a pending reservation."
                : cfg.label === "Booked" 
                  ? "This unit is fully booked."
                  : cfg.label === "Sold" 
                    ? "This unit has been sold."
                    : "This unit is currently unavailable.";

              return (
                <div className="status-popup-badge" style={{ background: `${cfg.color}18`, border: `1px solid ${cfg.color}44` }}>
                  <div className="status-popup-dot" style={{ background: cfg.color, boxShadow: `0 0 8px ${cfg.color}` }} />
                  <div>
                    <div className="status-popup-status" style={{ color: cfg.color }}>{cfg.label}</div>
            ...
            <div className="status-popup-actions">
              <button
                onClick={() => handleViewUnitDetails(popup.plotId)}
                className="status-popup-btn-primary"
              >
                View Unit Details →
              </button>
              <button
                onClick={() => setPopup(null)}
                className="status-popup-btn-secondary"
              >
                Close
              </button>
            </div>
      ...
      {/* Transition Overlay */}
      <div ref={overlayRef} className="floor-transition" />
    </div>
  );
};
