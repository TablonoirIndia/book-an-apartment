...
const TaxBreakdownBadge = ({ milestonePercent, feeBreakdown, sym = "₹" }) => {
  ...
  return (
    <span style={{ position: "relative", display: "inline-block" }}>
      <button type="button" className="tax-badge-btn"
        onClick={(e) => { e.stopPropagation(); setOpen((o) => !o); }}>
        +tax
      </button>
      {open && (
        <div className="tax-popover" onClick={(e) => e.stopPropagation()}>
          <div className="tax-popover-title">Charges for this instalment</div>
          <div className="tax-row"><span>Unit price portion</span><span>{f(data.base)}</span></div>
          {extraFees.map((fee, i) => (
            <div key={i} className="tax-row"><span>{fee.label}</span><span>{f(fee.val)}</span></div>
          ))}
          {data.tax > 0 && (
            <div className="tax-row highlight">
              <span>{data.taxLabel}{data.taxRate > 0 && <span style={{ opacity: 0.7, marginLeft: 4 }}>({data.taxRate}%{data.taxSystem ? ` · ${data.taxSystem.toUpperCase()}` : ""})</span>}</span>
              <span>{f(data.tax)}</span>
            </div>
          )}
          <div className="tax-row total"><span>Instalment total</span><span>{f(data.total)}</span></div>
          <button type="button" onClick={(e) => { e.stopPropagation(); setOpen(false); }}
            style={{ position: "absolute", top: 6, right: 8, background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 14 }}>×</button>
        </div>
      )}
    </span>
  );
...
const MilestoneTaxRow = ({ milestone, feeBreakdown }) => {
  ...
  return (
    <div style={{ background: "#fef9ee", border: "1px solid #fde68a", borderRadius: 8, padding: "10px 12px", marginTop: 6 }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: "#b45309", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 7 }}>
        Tax &amp; fees for this instalment
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <div className="tax-row"><span>Base ({milestone.percentage}% of unit price)</span><span>{f(data.base)}</span></div>
        {extraFees.map((fee, i) => <div key={i} className="tax-row"><span>{fee.label}</span><span>{f(fee.val)}</span></div>)}
        {data.tax > 0 && (
          <div className="tax-row highlight">
            <span>{data.taxLabel}{data.taxRate > 0 ? ` (${data.taxRate}%)` : ""}</span>
            <span>{f(data.tax)}</span>
...
const BookingSummaryTaxCard = ({ feeBreakdown, selectedPlan, plans, unit, sym = "₹" }) => {
  ...
  return (
    <div className="grand-total-banner" style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "#92400e", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 10 }}>
        📄 Payment Breakdown
      </div>
      <div className="gtb-row"><span className="gtb-label">Unit Price</span><span className="gtb-val">{f(feeBreakdown.unit_price)}</span></div>
      {extraFees.map((fee, i) => (
        <div key={i} className="gtb-row"><span className="gtb-label">{fee.label}</span><span className="gtb-val">{f(fee.val)}</span></div>
      ))}
      {feeBreakdown.tax_amount > 0 && (
        <div className="gtb-row" style={{ color: "#b45309", fontWeight: 600, borderTop: "1px solid #fde68a", paddingTop: 6, marginTop: 4 }}>
          <span>
            {feeBreakdown.tax_label || "Tax"}
            {feeBreakdown.tax_rate > 0 && <span style={{ fontSize: 10, marginLeft: 5, opacity: 0.8 }}>({feeBreakdown.tax_rate}%{feeBreakdown.tax_system ? ` · ${feeBreakdown.tax_system.toUpperCase()}` : ""}{feeBreakdown.tax_inclusive ? " · Incl." : ""})</span>}
          </span>
          <span>{f(feeBreakdown.tax_amount)}</span>
        </div>
      )}
      <div className="gtb-row" style={{ borderTop: "1.5px solid #fcd34d", paddingTop: 8, marginTop: 4 }}>
        <span className="gtb-total-label">Grand Total</span>
        <span className="gtb-total-val">{f(feeBreakdown.total_amount)}</span>
      </div>
      {plan && firstMs && (
        <div className="due-now-box">
          <div className="due-now-label">Due Now — {firstMs.label} ({firstMs.percentage}%)</div>
          <div className="due-now-val">{f(dueNow)}</div>
          <div className="due-now-note">Includes proportional taxes &amp; fees</div>
        </div>
      )}
...
/* ─────────────────────────────────────────────────────────────
   TYPE_COLORS / ICONS (payment milestone types)
───────────────────────────────────────────────────────────── */
const TYPE_COLORS = {
  down_payment:      { bg: "#eff6ff", color: "#1d4ed8", border: "#bfdbfe" },
  installment:       { bg: "#faf5ff", color: "#7e22ce", border: "#e9d5ff" },
  on_completion:     { bg: "#f0fdf4", color: "#15803d", border: "#bbf7d0" },
  on_handover:       { bg: "#fff7ed", color: "#c2410c", border: "#fed7aa" },
  rental_guarantee:  { bg: "#fff1f2", color: "#be123c", border: "#fecdd3" },
  custom:            { bg: "#f9fafb", color: "#374151", border: "#e5e7eb" },
};
const TYPE_ICONS = {
  down_payment: "💰", installment: "🔄", on_completion: "🏗️",
  on_handover: "🔑", rental_guarantee: "📈", custom: "⭐",
};
...
const UnitPage = () => {
  ...
  // ── Helpers ────────────────────────────────────────────────
  const fmtPrice       = (val) => val ? `${currency} ${Number(val).toLocaleString("en-IN")}` : null;
  const getStatusColor = (s) => { const v = s?.toLowerCase(); return v === "available" ? "#16a34a" : v === "pre-booked" ? "#d97706" : "#dc2626"; };

  // ── Guards ─────────────────────────────────────────────────
  if (isFetching) return (
    <div className="unit-loading-screen">
      <div style={{ fontSize: 48 }}>🏠</div>
      <div style={{ color: "var(--text-dark)", fontSize: 16, fontWeight: 600 }}>Loading unit…</div>
  ...
  if (fetchError || !unit) return (
    <div className="unit-loading-screen">
      ...
      <h2 style={{ color: "var(--text-dark)" }}>Unit Unavailable</h2>
      <p style={{ color: "var(--text-muted)" }}>{fetchError || "Unit not found."}</p>
      <button onClick={() => navigate(-1)} style={{ padding: "10px 24px", background: "var(--primary-color)", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700 }}>← Go Back</button>
    </div>
  );
  ...
  // ── Render panorama / gallery as full-screen ───────────────
  const isFullView = activeTab === "panorama" || (activeTab === "gallery" && hasGal);
  ...
  return (
    <div className="unit-page">
      ...
      {loginModalOpen && (
        <>
          <div className="up-modal-backdrop" onClick={closeModal} />
            ...
            {modalStep === "verify_email" ? (
              ...
              <>
                <div className="up-modal-tabs">
                  {["login", "register"].map((tab) => (
                    <button key={tab} className={`up-modal-tab${modalStep === tab ? " active" : ""}`}
                      onClick={() => { setModalStep(tab); setModalAuthError(""); }}>
                      {tab === "login" ? "Log In" : "Register"}
                    </button>
                  ))}
                </div>
                <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 16, lineHeight: 1.6 }}>
                  {modalStep === "login" ? "Log in to save this unit to your wishlist." : "Create an account to save this unit."}
                </p>
                {modalStep === "login"
                  ? <StepLogin form={loginForm} setForm={setLoginForm} onLogin={handleModalLogin} onSwitch={() => { setModalStep("register"); setModalAuthError(""); }} loading={authLoading} error={modalAuthError} />
                  : <StepRegister form={registerForm} setForm={setRegisterForm} onRegister={handleModalRegister} onSwitch={() => { setModalStep("login"); setModalAuthError(""); }} loading={authLoading} error={modalAuthError} />}
      ...
      <div className="unit-header">
        ...
        <div className="unit-info">
          <div className="unit-title-row">
            <span className="unit-name">Unit {unit.plot_number}</span>
            <span className="unit-status-badge" style={{ background: statusColor + "18", color: statusColor, border: `1px solid ${statusColor}44` }}>{unit.status}</span>
          </div>
          <div className="unit-subtitle">{unit.plot_type}{unit.plot_size ? ` · ${unit.plot_size} sqft` : ""}{unit.direction ? ` · ${unit.direction}` : ""}</div>
        </div>

        <div className="unit-tabs">
          <button className={`unit-tab${activeTab === "details"  ? " active" : ""}`} onClick={switchToDetails}>📋 Details</button>
          {hasPlans && <button className={`unit-tab${activeTab === "payment" ? " active" : ""}`} onClick={() => { destroyViewer(); setActiveTab("payment"); }}>💳 Payment Plans</button>}
          {hasPano  && <button className={`unit-tab${activeTab === "panorama" ? " active" : ""}`} onClick={switchToPanorama}>🌐 360°</button>}
          {hasGal   && <button className={`unit-tab${activeTab === "gallery"  ? " active" : ""}`} onClick={switchToGallery}>🖼 Gallery ({gallery.length})</button>}
        </div>
        ...
        <div className="unit-price-wrap">
          {user && (
            <button onClick={() => navigate("/dashboard")} title={`${user.name} — My Dashboard`}
              style={{ display: "flex", alignItems: "center", gap: 7, background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 20, padding: "5px 14px 5px 6px", cursor: "pointer", color: "var(--text-dark)", fontSize: 12, fontWeight: 700, flexShrink: 0, fontFamily: "var(--font-body)", transition: "border-color 0.2s" }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = "var(--primary-color)")}
              onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--card-border)")}>
              <span style={{ width: 24, height: 24, borderRadius: "50%", background: "var(--primary-color)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{user.name?.[0]?.toUpperCase()}</span>
              <span style={{ whiteSpace: "nowrap" }}>Dashboard</span>
            </button>
          )}
          <div className="unit-price-box">
            {unit.offer_price && <div className="unit-offer-price">{fmtPrice(unit.offer_price)}</div>}
            <div className="unit-actual-price" style={{ color: unit.offer_price ? "var(--text-muted)" : "var(--text-dark)", textDecoration: unit.offer_price ? "line-through" : "none", fontSize: unit.offer_price ? 11 : 14 }}>{fmtPrice(unit.actual_price)}</div>
            {feeBreakdown?.tax_enabled && feeBreakdown?.tax_rate > 0 && (
              <div style={{ fontSize: 10, color: "#b45309", marginTop: 2, fontWeight: 600 }}>+{feeBreakdown.tax_rate}% {feeBreakdown.tax_label || "Tax"}</div>
            )}
          </div>
          {isAvail && <button className="book-now-btn" onClick={openBooking}>Book Now</button>}
          {unit && (
            <button
              ...
              title={isWished ? "Remove from Wishlist" : "Save to Wishlist"}
              style={{ background: isWished ? "#fff5f5" : "var(--page-bg)", border: `1.5px solid ${isWished ? "#fecaca" : "var(--card-border)"}`, borderRadius: 8, cursor: "pointer", padding: "6px 10px", fontSize: 18, lineHeight: 1, transition: "all 0.2s", color: isWished ? "#ef4444" : "var(--text-muted)" }}>
              {isWished ? "❤️" : "🤍"}
            </button>
          )}
        </div>
      ...
      <div className="unit-content-area">
        ...
        {/* Two-column layout for details / payment / gallery */}
        {activeTab !== "panorama" && (
          <div className="unit-detail-layout">
            ...
            <div className="unit-detail-left">
              ...
              {/* Gallery strip (shown on details, gallery, payment tabs) */}
              {(activeTab === "details" || activeTab === "gallery" || activeTab === "payment") && (hasGal || hasPano) && (
                <GalleryStrip
                  gallery={gallery}
                  hasPano={hasPano}
                  plotId={plotId}
                  ...
                  galIdx={galIdx}
                  setGalIdx={setGalIdx}
                  onOpenPano={switchToPanorama}
                />
              )}

              {activeTab === "details" && (
                <UnitDetailsContent unit={unit} fmtPrice={fmtPrice} statusColor={statusColor} currency={currency} sym={sym} feeBreakdown={feeBreakdown} />
              )}

              {activeTab === "payment" && (
                <PaymentPlansContent
                  plans={unit.payment_plans || []}
                  fmtPrice={fmtPrice}
                  currency={currency}
                  unitPrice={unit.offer_price || unit.actual_price}
                  feeBreakdown={feeBreakdown}
                  sym={sym}
                />
              )}

              {activeTab === "gallery" && hasGal && (
                <div style={{ marginTop: 16 }}>
                  <div className="up-section">
                    <div className="up-section-head">🖼 All Photos ({gallery.length})</div>
                    <div className="up-section-body">
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
                        {gallery.map((img, i) => (
                          <img key={i} src={img} alt={`Gallery ${i + 1}`}
                            onClick={() => { setGalIdx(i); navigate(`/unit/${plotId}/gallery/${i}`); }}
                            style={{ width: "100%", aspectRatio: "4/3", objectFit: "cover", borderRadius: 8, cursor: "pointer", border: `2px solid ${i === galIdx ? "var(--primary-color)" : "transparent"}`, transition: "all 0.2s" }} />
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <div className="unit-detail-right">
              <div className="right-panel-header">
                <div className="right-panel-price">
                  {unit.offer_price ? fmtPrice(unit.offer_price) : fmtPrice(unit.actual_price)}
                </div>
                {unit.offer_price && <div className="right-panel-price-orig">{fmtPrice(unit.actual_price)}</div>}
                {feeBreakdown?.tax_enabled && feeBreakdown?.total_amount > 0 && (
                  <div style={{ fontSize: 12, color: "#b45309", fontWeight: 600, marginTop: 3 }}>
                    Grand Total: {fmtExact(feeBreakdown.total_amount, sym)} (incl. tax)
                  </div>
                )}
                <div className="right-panel-title">Unit {unit.plot_number}</div>
                <div className="right-panel-sub">{unit.plot_type}{unit.plot_size ? ` · ${unit.plot_size} sqft` : ""}{unit.direction ? ` · ${unit.direction}` : ""}</div>
                <div style={{ marginTop: 8, display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 12px", borderRadius: 20, fontSize: 12, fontWeight: 700, background: statusColor + "18", color: statusColor, border: `1px solid ${statusColor}44` }}>
                  ● {unit.status}
                </div>
              </div>
              ...
              <div style={{ padding: "14px 22px 0" }}>
                <div className="rp-stats">
                  {unit.bedrooms  != null && <div className="rp-stat"><span className="rp-stat-icon">🛏</span><div className="rp-stat-val">{unit.bedrooms}</div><div className="rp-stat-lbl">Beds</div></div>}
                  {unit.bathrooms != null && <div className="rp-stat"><span className="rp-stat-icon">🚿</span><div className="rp-stat-val">{unit.bathrooms}</div><div className="rp-stat-lbl">Baths</div></div>}
                  {unit.plot_size  && <div className="rp-stat"><span className="rp-stat-icon">📐</span><div className="rp-stat-val">{unit.plot_size}</div><div className="rp-stat-lbl">sqft</div></div>}
                  {!unit.bedrooms && !unit.bathrooms && !unit.plot_size && (
                    <>
                      <div className="rp-stat"><span className="rp-stat-icon">🏢</span><div className="rp-stat-val">{unit.plot_type || "—"}</div><div className="rp-stat-lbl">Type</div></div>
                      <div className="rp-stat"><span className="rp-stat-icon">🧭</span><div className="rp-stat-val">{unit.direction || "—"}</div><div className="rp-stat-lbl">Facing</div></div>
                      <div className="rp-stat"><span className="rp-stat-icon">🚪</span><div className="rp-stat-val">{unit.rooms?.length || 0}</div><div className="rp-stat-lbl">Rooms</div></div>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="right-panel-scroll">
                {bookingStep === "broker_inactive" ? <BrokerInactiveScreen onClose={() => setSidebarOpen(false)} />
                : bookingDone                      ? <BookingDone unit={unit} onClose={() => setSidebarOpen(false)} />
                : bookingStep === "details"         ? <StepDetails user={user} onBook={() => user ? (hasPlans ? setBookingStep("plan") : setBookingStep("form")) : setBookingStep("login")} onLogout={logout} isAvail={isAvail} />
                : bookingStep === "login"           ? <StepLogin form={loginForm} setForm={setLoginForm} onLogin={handleLogin} onSwitch={() => { setBookingStep("register"); setAuthError(""); }} loading={authLoading} error={authError} />
                : bookingStep === "register"        ? <StepRegister form={registerForm} setForm={setRegisterForm} onRegister={handleRegister} onSwitch={() => { setBookingStep("login"); setAuthError(""); }} loading={authLoading} error={authError} />
                : bookingStep === "verify_email"    ? <StepOtp key="otp-email" type="email" onVerify={handleVerifyEmail} onResend={() => handleResendOtp("email")} loading={authLoading} error={authError} setError={setAuthError} />
                : bookingStep === "verify_phone"    ? <StepOtp key="otp-phone" type="phone" onVerify={handleVerifyPhone} onResend={() => handleResendOtp("phone")} loading={authLoading} error={authError} setError={setAuthError} />
...
/* ═════════════════════════════════════════════════════════════
   GalleryStrip — Dubizzle-style image strip
═════════════════════════════════════════════════════════════ */
const GalleryStrip = ({ gallery, hasPano, plotId, navigate, galIdx, setGalIdx, onOpenPano }) => {
  if (!gallery.length && !hasPano) return null;
  const total = gallery.length;
  return (
    <div className="up-gallery">
      <div className="up-gallery-main" onClick={() => total > 0 && navigate(`/unit/${plotId}/gallery/${galIdx}`)}>
        {total > 0
          ? <img src={gallery[galIdx]} alt={`View ${galIdx + 1}`} />
          : <div style={{ width: "100%", height: "100%", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.3)", fontSize: 40, aspectRatio: "16/9" }}>🏠</div>}

        <div className="up-gallery-badges">
          {hasPano && (
            <button className="up-gallery-badge pano" onClick={(e) => { e.stopPropagation(); onOpenPano(); }}>
              🌐 360° View
            </button>
          )}
        </div>
        ...
        {total > 1 && (
          <>
            <button className="up-gallery-nav prev" onClick={(e) => { e.stopPropagation(); setGalIdx((i) => (i - 1 + total) % total); }}>‹</button>
            <button className="up-gallery-nav next" onClick={(e) => { e.stopPropagation(); setGalIdx((i) => (i + 1) % total); }}>›</button>
            <div className="up-gallery-counter">📷 {galIdx + 1} / {total}</div>
          </>
        )}
      </div>
      {total > 1 && (
        <div className="up-gallery-thumbs">
          {gallery.map((img, i) => (
            <img key={i} src={img} alt="" className={i === galIdx ? "active" : ""} onClick={() => setGalIdx(i)} />
          ))}
        </div>
      )}
    </div>
...
/* ═════════════════════════════════════════════════════════════
   UnitDetailsContent — Dubizzle-style detail sections
═════════════════════════════════════════════════════════════ */
const UnitDetailsContent = ({ unit, fmtPrice, statusColor, currency, sym, feeBreakdown }) => {
  const hasDims     = unit.room_dimensions?.length > 0;
  ...
  return (
    <>
      ...
      <div className="up-headline">
        <div>
          <h1 className="up-title">Unit {unit.plot_number} — {unit.plot_type}</h1>
          <div className="up-meta-chips">
            {unit.bedrooms  != null && <span className="up-meta-chip"><span className="chip-icon">🛏</span>{unit.bedrooms} Bed{unit.bedrooms !== 1 ? "s" : ""}</span>}
            {unit.bathrooms != null && <span className="up-meta-chip"><span className="chip-icon">🚿</span>{unit.bathrooms} Bath{unit.bathrooms !== 1 ? "s" : ""}</span>}
            {unit.plot_size          && <span className="up-meta-chip"><span className="chip-icon">📐</span>{unit.plot_size} sqft</span>}
            {unit.direction          && <span className="up-meta-chip"><span className="chip-icon">🧭</span>{unit.direction}</span>}
          </div>
        </div>
        <div className="up-price-block">
          {unit.offer_price && <div className="up-price-main">{fmtPrice(unit.offer_price)}</div>}
          {unit.offer_price && <div className="up-price-original">{fmtPrice(unit.actual_price)}</div>}
          {!unit.offer_price && unit.actual_price && <div className="up-price-main">{fmtPrice(unit.actual_price)}</div>}
          {feeBreakdown?.tax_enabled && feeBreakdown?.tax_rate > 0 && (
            <div className="up-price-tax-note">+{feeBreakdown.tax_rate}% {feeBreakdown.tax_label || "Tax"}</div>
          )}
          <div className="up-price-status" style={{ background: statusColor + "18", color: statusColor, border: `1px solid ${statusColor}44` }}>● {unit.status}</div>
        </div>
      </div>

      {/* Overview facts */}
      <div className="up-section">
        <div className="up-section-head">🏠 Overview</div>
        <div className="up-section-body">
          <div className="up-facts-grid">
            {[
              ["Unit No",    unit.plot_number],
              ["Type",       unit.plot_type],
              ...
              <div key={lbl} className="up-fact">
                <div className="up-fact-label">{lbl}</div>
                <div className="up-fact-value">{val}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {hasAreaData && (
        <div className="up-section">
          <div className="up-section-head">📐 Floor Areas</div>
          <div className="up-section-body">
            <div className="up-area-grid">
              {unit.internal_area_sqft && (
                <div className="up-area-card" style={{ borderColor: "var(--primary-color)33" }}>
                  <div className="up-area-label">Internal Area</div>
                  <div className="up-area-sqft" style={{ color: "var(--primary-color)" }}>{Number(unit.internal_area_sqft).toLocaleString()} <span>sq ft</span></div>
                  <div className="up-area-sqm">{unit.internal_area_sqm ? Number(unit.internal_area_sqm).toLocaleString() : (unit.internal_area_sqft / 10.764).toFixed(1)} sq m</div>
                </div>
              )}
              {unit.external_area_sqft && (
                <div className="up-area-card">
                  <div className="up-area-label">External Area</div>
                  <div className="up-area-sqft">{Number(unit.external_area_sqft).toLocaleString()} <span>sq ft</span></div>
                  <div className="up-area-sqm">{unit.external_area_sqm ? Number(unit.external_area_sqm).toLocaleString() : (unit.external_area_sqft / 10.764).toFixed(1)} sq m</div>
                </div>
              )}
              {unit.internal_area_sqft && unit.external_area_sqft && (
                <div className="up-area-card" style={{ borderColor: "#86efac" }}>
                  <div className="up-area-label">Total Area</div>
                  <div className="up-area-sqft" style={{ color: "#16a34a" }}>{(parseFloat(unit.internal_area_sqft) + parseFloat(unit.external_area_sqft)).toFixed(2)} <span>sq ft</span></div>
                  {unit.internal_area_sqm && unit.external_area_sqm && <div className="up-area-sqm">{(parseFloat(unit.internal_area_sqm) + parseFloat(unit.external_area_sqm)).toFixed(2)} sq m</div>}
                </div>
              )}
            </div>
          </div>
      ...
      {hasDims && (
        <div className="up-section">
          <div className="up-section-head">📏 Room Dimensions</div>
          <div className="up-section-body">
            {unit.room_dimensions.map((dim, i) => {
              const u = dim.unit || "m", w = parseFloat(dim.width), l = parseFloat(dim.length);
              const wFt = u === "m" ? toFt(w) : w.toFixed(1), lFt = u === "m" ? toFt(l) : l.toFixed(1);
              const wM  = u === "ft" ? (w / 3.28084).toFixed(1) : w.toFixed(1), lM = u === "ft" ? (l / 3.28084).toFixed(1) : l.toFixed(1);
              const sqm  = dim.area_sqm  ? parseFloat(dim.area_sqm)  : w && l ? u === "m" ? w * l : (w * l) / 10.7639 : null;
              const sqft = dim.area_sqft ? parseFloat(dim.area_sqft) : sqm ? sqm * 10.7639 : null;
              return (
                <div key={i} className="up-dim-row">
                  <div className="up-dim-name">{dim.label}</div>
                  <div className="up-dim-vals">
                    <span className="up-dim-primary">{wM} × {lM} m</span>
                    <span className="up-dim-secondary">{wFt}' × {lFt}'</span>
                    {sqm != null && <span className="up-dim-area">{sqm.toFixed(1)} m² / {sqft.toFixed(1)} ft²</span>}
                  </div>
                </div>
              );
            })}
      ...
      {hasAmenities && (
        <div className="up-section">
          <div className="up-section-head">✨ Amenities</div>
          <div className="up-section-body">
            <div className="up-amenities">
              {unit.amenities.map((a, i) => (
                <div key={i} className="up-amenity">
                  <span>{amenityIcon(a)}</span><span>{a}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {unit.floor_plan_image && (
        <div className="up-section">
          <div className="up-section-head">🗺️ Floor Plan</div>
          <div className="up-section-body up-floor-plan">
            <img src={unit.floor_plan_image} alt="Floor Plan" />
          </div>
        </div>
      )}
    </>
...
/* ═════════════════════════════════════════════════════════════
   PaymentPlansContent
═════════════════════════════════════════════════════════════ */
const PaymentPlansContent = ({ plans, fmtPrice, currency, unitPrice, feeBreakdown, sym = "₹" }) => {
  const [activePlan, setActivePlan] = useState(0);
  if (!plans?.length) return (
    <div className="up-section">
      <div className="up-section-body" style={{ textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: 40, marginBottom: 12 }}>💳</div>
        <div style={{ fontSize: 15, fontWeight: 600, color: "var(--text-muted)" }}>No payment plans available</div>
      </div>
    </div>
  );

  const basePrice = parseFloat(String(unitPrice || "0").replace(/[^0-9.]/g, "")) || 0;
  const calcAmtLabel = (pct) => basePrice > 0 ? fmtExact((basePrice * parseFloat(pct)) / 100, sym) : null;
  const plan     = plans[activePlan];
  const totalPct = plan.milestones.reduce((s, m) => s + (parseFloat(m.percentage) || 0), 0);

  return (
    <div className="up-plans-wrap">
      {plans.length > 1 && (
        <div className="up-plan-tabs">
          {plans.map((p, i) => (
            <button key={p.id} className={`up-plan-tab${i === activePlan ? " active" : ""}`} onClick={() => setActivePlan(i)}>{p.name}</button>
          ))}
        </div>
      )}
      ...
      {feeBreakdown?.tax_enabled && feeBreakdown?.total_amount > 0 && (
        <div className="grand-total-banner" style={{ marginBottom: 16 }}>
          <div className="gtb-row">
            <span className="gtb-label">Unit {fmtExact(basePrice, sym)} + {feeBreakdown.tax_label || "Tax"} ({feeBreakdown.tax_rate}%) + fees</span>
            <span className="gtb-total-val">Grand Total: {fmtExact(feeBreakdown.total_amount, sym)}</span>
          </div>
        </div>
      )}
      ...
      <div className="ms-card">
        <div className="ms-header">
          <div>
            <div style={{ fontFamily: "var(--font-heading)", fontSize: 17, fontWeight: 700, color: "var(--text-dark)" }}>{plan.name}</div>
            {plan.description && <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 2 }}>{plan.description}</div>}
          </div>
          {totalPct === 100 && <span style={{ padding: "3px 12px", borderRadius: 20, background: "#f0fdf4", border: "1px solid #bbf7d0", color: "#16a34a", fontSize: 12, fontWeight: 700 }}>✓ 100%</span>}
        </div>
        ...
        {plan.milestones.map((m, i) => {
          ...
          const icon  = TYPE_ICONS[m.type]  || "⭐";
          const isLast = i === plan.milestones.length - 1;
          const displayAmt = m.calculated_amount ? fmtExact(m.calculated_amount, sym) : m.percentage ? calcAmtLabel(m.percentage) : m.fixed_amount ? fmtExact(m.fixed_amount, sym) : null;
          const msTaxData  = feeBreakdown && m.percentage ? computeMilestoneAmounts(m.percentage, feeBreakdown) : null;

          return (
            <div key={m.id || i} className="ms-row">
              <div className="ms-num" style={{ borderColor: c.border, color: c.color }}>{i + 1}</div>
              <div className="ms-body">
                <div className="ms-type-chip" style={{ background: c.bg, color: c.color }}>{icon} {m.type_label || m.type?.replace(/_/g, " ")}</div>
                <div className="ms-label">{m.label}</div>
                {(m.due_date || m.due_after_days) && (
                  <div className="ms-due">📅 {m.due_date ? `Due by ${m.due_date}` : `Due ${m.due_after_days} days after booking`}</div>
                )}
                {m.note && <div className="ms-note">{m.note}</div>}
                {feeBreakdown && m.percentage && <MilestoneTaxRow milestone={m} feeBreakdown={feeBreakdown} />}
              </div>
              <div className="ms-amounts">
                {m.percentage > 0 && <div className="ms-pct">{m.percentage}%</div>}
                {displayAmt && (
                  <div className="ms-amt" style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                    ≈ {displayAmt}
                    {msTaxData && (msTaxData.tax > 0 || msTaxData.fees > 0) && (
                      <TaxBreakdownBadge milestonePercent={m.percentage} feeBreakdown={feeBreakdown} sym={sym} />
                    )}
                ...
                {msTaxData && msTaxData.total > 0 && <div className="ms-total">Total: {fmtExact(msTaxData.total, sym)}</div>}
              </div>
            </div>
          );
        })}
        ...
        <div className="ms-footer">
          <span style={{ fontSize: 13, color: "var(--text-muted)", fontWeight: 600 }}>Total payment mapped</span>
          <div style={{ textAlign: "right" }}>
            <span style={{ fontSize: 18, fontWeight: 800, color: totalPct === 100 ? "#16a34a" : totalPct > 100 ? "#dc2626" : "var(--primary-color)" }}>
              {totalPct.toFixed(0)}%{totalPct === 100 ? " ✓" : ""}
            </span>
            {feeBreakdown?.total_amount > 0 && (
              <div style={{ fontSize: 11, color: "#b45309", marginTop: 2 }}>= {fmtExact(feeBreakdown.total_amount, sym)} (incl. all taxes &amp; fees)</div>
            )}
          </div>
        </div>
      </div>
...
const BrokerInactiveScreen = ({ onClose }) => (
  <div className="broker-inactive">
    <div className="broker-inactive-icon">🔒</div>
    <div className="broker-inactive-title">Account Inactive</div>
    <p style={{ color: "var(--text-muted)", fontSize: 13, lineHeight: 1.7, marginBottom: 20 }}>
      Your broker account is currently <strong style={{ color: "#dc2626" }}>inactive</strong>. Please contact the admin to activate your account.
    </p>
    <div style={{ background: "#fff5f5", border: "1.5px solid #fecaca", borderRadius: 10, padding: "12px 16px", marginBottom: 20, fontSize: 12, color: "#dc2626", textAlign: "left" }}>
      <div style={{ fontWeight: 700, marginBottom: 4 }}>What to do next:</div>
      <ul style={{ margin: 0, paddingLeft: 18, lineHeight: 1.8 }}>
        <li>Contact your project administrator</li>
        <li>Ask them to activate your broker account</li>
        <li>You can book once your status is active</li>
      </ul>
    </div>
    <button onClick={onClose} className="book-btn" style={{ background: "var(--page-bg)", color: "var(--text-muted)", border: "1.5px solid var(--card-border)" }}>Close</button>
  </div>
);

const StepDetails = ({ user, onBook, onLogout, isAvail }) => (
  <div>
    {user
      ? <div className="logged-in-card">
          <div>
            <div className="logged-in-label">✓ Logged in as</div>
            <div className="logged-in-name">{user.name}</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{user.email}</div>
          </div>
          <button className="logout-btn" onClick={onLogout}>Logout</button>
        </div>
      : <p style={{ color: "var(--text-muted)", fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>
          To book this unit, please log in or create an account. Both email and phone will be verified.
        </p>}
    {isAvail !== false && (
      <button className="book-btn" onClick={onBook}>
        {user ? "Continue to Booking →" : "Login / Register to Book →"}
      </button>
    )}
  </div>
);
...
const StepLogin = ({ form, setForm, onLogin, onSwitch, loading, error }) => (
  <div>
    <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 15, marginBottom: 16 }}>Login to your account</div>
    {error && <div className="error-box">⚠️ {error}</div>}
    <input className="sidebar-input" type="email" placeholder="Email address" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} />
    <input className="sidebar-input" type="password" placeholder="Password" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} onKeyDown={(e) => e.key === "Enter" && onLogin()} style={{ marginBottom: 16 }} />
    <button className="book-btn" onClick={onLogin} disabled={loading || !form.email || !form.password}>{loading ? "Logging in…" : "Login →"}</button>
    {onSwitch && <p style={{ textAlign: "center", marginTop: 14, color: "var(--text-muted)", fontSize: 13 }}>Don't have an account? <button className="auth-link" onClick={onSwitch}>Register</button></p>}
  </div>
);
...
const StepRegister = ({ form, setForm, onRegister, onSwitch, loading, error }) => {
  const [showPass, setShowPass] = useState(false);
  const strength = !form.password ? 0 : [form.password.length >= 8, /[A-Z]/.test(form.password), /[0-9]/.test(form.password), /[^A-Za-z0-9]/.test(form.password)].filter(Boolean).length;
  const strengthColor = ["#ef4444", "#f59e0b", "#f59e0b", "#16a34a", "#16a34a"][strength];
  return (
    <div>
      <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Create an account</div>
      <div style={{ color: "var(--text-muted)", fontSize: 12, marginBottom: 14 }}>We'll verify your email and phone number</div>
      {error && <div className="error-box">⚠️ {error}</div>}
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        {[{ v: "user", label: "👤 Buyer" }, { v: "broker", label: "🤝 Broker" }].map(({ v, label }) => (
          <button key={v} type="button" onClick={() => setForm((f) => ({ ...f, role: v }))}
            style={{ flex: 1, padding: 8, borderRadius: 8, cursor: "pointer", fontSize: 13, border: `1.5px solid ${form.role === v ? "var(--primary-color)" : "var(--card-border)"}`, background: form.role === v ? "var(--page-bg)" : "transparent", color: form.role === v ? "var(--primary-color)" : "var(--text-muted)", fontWeight: form.role === v ? 700 : 400, fontFamily: "var(--font-body)" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <CountryCodePicker value={form.country_code} onChange={(v) => setForm((f) => ({ ...f, country_code: v }))} />
        <input style={{ flex: 1, padding: "10px 12px", background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 8, color: "var(--text-dark)", fontSize: 13, fontFamily: "var(--font-body)", outline: "none" }} type="tel" placeholder="Phone number *" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value.replace(/\D/g, "") }))} />
      </div>
      <div style={{ position: "relative", marginBottom: 4 }}>
        <input style={{ width: "100%", padding: "10px 40px 10px 12px", background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 8, color: "var(--text-dark)", fontSize: 13, boxSizing: "border-box", fontFamily: "var(--font-body)", outline: "none" }} type={showPass ? "text" : "password"} placeholder="Password (min 8 chars) *" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} />
        <button type="button" onClick={() => setShowPass((s) => !s)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "var(--text-muted)", cursor: "pointer", fontSize: 14 }}>{showPass ? "🙈" : "👁"}</button>
      </div>
      {form.password && (
        <div style={{ marginBottom: 12 }}>
          <div className="pw-strength">{[1,2,3,4].map((i) => <div key={i} className="pw-bar" style={{ background: i <= strength ? strengthColor : "var(--card-border)" }} />)}</div>
          <div style={{ fontSize: 11, color: strengthColor }}>{["","Weak","Fair","Good","Strong"][strength]}</div>
        </div>
      )}
      <button className="book-btn" style={{ marginTop: 6 }} onClick={onRegister} disabled={loading || !form.name || !form.email || !form.phone || !form.password || form.password.length < 8}>{loading ? "Creating account…" : "Register & Verify →"}</button>
      <p style={{ textAlign: "center", marginTop: 14, color: "var(--text-muted)", fontSize: 13 }}>Already have an account? <button className="auth-link" onClick={onSwitch}>Login</button></p>
    </div>
  );
};
  ...
  return (
    <div>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        <div style={{ fontSize: 36, marginBottom: 8 }}>{isEmail ? "📧" : "📱"}</div>
        <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 15 }}>Verify your {isEmail ? "email" : "phone"}</div>
        <div style={{ color: "var(--text-muted)", fontSize: 12, marginTop: 6 }}>Enter the 6-digit code sent to your {isEmail ? "email" : "phone"}.</div>
      </div>
      {error && <div style={{ padding: "8px 12px", borderRadius: 8, marginBottom: 12, fontSize: 12, background: error.startsWith("✓") ? "#f0fdf4" : "#fff5f5", border: `1px solid ${error.startsWith("✓") ? "#bbf7d0" : "#fecaca"}`, color: error.startsWith("✓") ? "#16a34a" : "#dc2626" }}>{error}</div>}
      ...
      <div style={{ textAlign: "center", marginTop: 16 }}>
        <span style={{ color: "var(--text-muted)", fontSize: 13 }}>Didn't receive the code? </span>
        <button className="auth-link" onClick={() => { setOtp(["","","","","",""]); onResend(); }} disabled={loading}>Resend</button>
      </div>
    </div>
  );
};
...
const StepPlanSelect = ({ plans, selected, onSelect, fmtPrice, unitPrice, onNext, onBack, existingBookingId, feeBreakdown, sym = "₹" }) => {
  const [expanded, setExpanded] = useState(null);
  const price = parseFloat(String(unitPrice || "0").replace(/,/g, "")) || 0;
  const calcAmtLabel = (pct) => price > 0 ? fmtExact((price * parseFloat(pct)) / 100, sym) : null;

  return (
    <div>
      <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 15, marginBottom: 4 }}>Choose a Payment Plan</div>
      <div style={{ color: "var(--text-muted)", fontSize: 12, marginBottom: 14 }}>Select how you'd like to pay for this unit</div>

      {feeBreakdown?.tax_enabled && feeBreakdown?.total_amount > 0 && (
        <div className="grand-total-banner" style={{ marginBottom: 14 }}>
          <div className="gtb-row"><span className="gtb-label">Unit Price</span><span className="gtb-val">{fmtExact(feeBreakdown.unit_price || price, sym)}</span></div>
          {feeBreakdown.tax_amount > 0 && (
            <div className="gtb-row"><span style={{ color: "#b45309" }}>{feeBreakdown.tax_label || "Tax"} ({feeBreakdown.tax_rate}%)</span><span style={{ color: "#b45309", fontWeight: 700 }}>{fmtExact(feeBreakdown.tax_amount, sym)}</span></div>
          )}
          <div className="gtb-row" style={{ borderTop: "1px solid #fde68a", paddingTop: 5, marginTop: 3 }}>
            <span className="gtb-total-label">Grand Total</span>
            <span className="gtb-total-val">{fmtExact(feeBreakdown.total_amount, sym)}</span>
          </div>
        </div>
      )}

      {/* Full payment option */}
      <div className={`plan-option${selected === null ? " selected" : ""}`} onClick={() => onSelect(null)}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <div style={{ color: "var(--text-dark)", fontWeight: 600, fontSize: 13 }}>Full Payment</div>
            <div style={{ color: "var(--text-muted)", fontSize: 11, marginTop: 2 }}>
              Pay {feeBreakdown?.total_amount ? `${fmtExact(feeBreakdown.total_amount, sym)} (incl. taxes)` : "the full amount"} upfront
            </div>
          </div>
          <div className={`plan-radio${selected === null ? " selected" : ""}`} />
        </div>
      </div>
      ...
      {plans.map((plan) => {
        ...
        return (
          <div key={plan.id} style={{ marginBottom: 8 }}>
            <div className={`plan-option${isSelected ? " selected" : ""}`} style={{ borderRadius: isExpanded ? "8px 8px 0 0" : 8 }}
              onClick={() => { onSelect(plan.id); setExpanded(isExpanded ? null : plan.id); }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 13 }}>{plan.name}</div>
                  {plan.description && <div style={{ color: "var(--text-muted)", fontSize: 11, marginTop: 2 }}>{plan.description}</div>}
                  {!isExpanded && (
                    <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4 }}>
                      {plan.milestones.slice(0, 2).map((m, i) => (
                        <span key={i} style={{ fontSize: 10, padding: "2px 8px", borderRadius: 8, background: "var(--page-bg)", color: "var(--primary-color)", fontWeight: 600, border: "1px solid var(--card-border)" }}>
                          {m.label}{m.percentage ? ` · ${m.percentage}%` : ""}
                        </span>
                      ))}
                      {plan.milestones.length > 2 && <span style={{ fontSize: 10, color: "var(--text-muted)" }}>+{plan.milestones.length - 2} more</span>}
                    </div>
                  )}
                </div>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
                  <div className={`plan-radio${isSelected ? " selected" : ""}`} />
                  <button type="button" onClick={(e) => { e.stopPropagation(); setExpanded(isExpanded ? null : plan.id); }}
                    style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: 11, cursor: "pointer", padding: 0, fontFamily: "var(--font-body)" }}>
            ...
            {isExpanded && (
              <div style={{ background: "var(--page-bg)", borderRadius: "0 0 8px 8px", border: "1.5px solid var(--primary-color)44", borderTop: "none" }}>
                {plan.milestones.map((m, i) => {
                  const c    = TYPE_COLORS[m.type] || TYPE_COLORS.custom;
                  const icon = TYPE_ICONS[m.type]  || "⭐";
                  const amt  = m.calculated_amount ? fmtExact(m.calculated_amount, sym) : m.percentage ? calcAmtLabel(m.percentage) : null;
                  return (
                    <div key={i} style={{ padding: "10px 14px", borderBottom: i < plan.milestones.length - 1 ? "1px solid var(--card-border)" : "none" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
                          <span style={{ fontSize: 16 }}>{icon}</span>
                          <div>
                            <span style={{ fontSize: 10, padding: "1px 7px", borderRadius: 8, background: c.bg, color: c.color, fontWeight: 700 }}>{m.type_label}</span>
                            <span style={{ color: "var(--text-dark)", fontSize: 12, fontWeight: 600, marginLeft: 6 }}>{m.label}</span>
                          </div>
                        </div>
                        <div style={{ textAlign: "right" }}>
                          {m.percentage > 0 && <div style={{ color: "var(--primary-color)", fontWeight: 800, fontSize: 15 }}>{m.percentage}%</div>}
                          {amt && <div style={{ color: "var(--text-muted)", fontSize: 11 }}>≈ {amt}</div>}
                        </div>
                      </div>
                      {feeBreakdown && m.percentage && <MilestoneTaxRow milestone={m} feeBreakdown={feeBreakdown} />}
                    </div>
                ...
                <div style={{ padding: "8px 14px", display: "flex", justifyContent: "space-between", borderTop: "1px solid var(--card-border)", background: "var(--card-bg)" }}>
                  <span style={{ color: "var(--text-muted)", fontSize: 12 }}>Total</span>
                  <div style={{ textAlign: "right" }}>
                    <span style={{ color: totalPct === 100 ? "#16a34a" : "var(--primary-color)", fontWeight: 700, fontSize: 13 }}>{totalPct.toFixed(0)}%{totalPct === 100 ? " ✓" : ""}</span>
                    {feeBreakdown?.total_amount > 0 && (
                      <div style={{ fontSize: 10, color: "#b45309", marginTop: 2 }}>= {fmtExact(feeBreakdown.total_amount, sym)} incl. taxes</div>
                    )}
                  </div>
                </div>
              </div>
      ...
      <button className="book-btn" onClick={onNext} style={{ marginTop: 12 }}>{existingBookingId ? "Update Plan & Pay →" : "Continue →"}</button>
      <button type="button" onClick={onBack} style={{ display: "block", width: "100%", marginTop: 8, background: "none", border: "1.5px solid var(--card-border)", borderRadius: 8, color: "var(--text-muted)", padding: 10, cursor: "pointer", fontSize: 13, fontFamily: "var(--font-body)" }}>← Back</button>
    </div>
  );
};
...
const StepBookingForm = ({ user, form, setForm, onSubmit, loading, error, selectedPlan, plans, onBack, unit, existingBookingId, feeBreakdown, sym = "₹" }) => {
  ...
  return (
    <div>
      <div style={{ color: "var(--text-dark)", fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{existingBookingId ? "Update Payment Plan" : "Confirm Booking"}</div>
      <div style={{ color: "var(--text-muted)", fontSize: 12, marginBottom: 12 }}>
        {existingBookingId
          ? <span>Updating plan for booking <span style={{ color: "var(--primary-color)" }}>#{existingBookingId}</span></span>
          : isBroker
            ? <>Booking as broker <span style={{ color: "var(--primary-color)" }}>{user?.name}</span></>
            : <>Booking as <span style={{ color: "var(--primary-color)" }}>{user?.name}</span></>}
      </div>

      {plan && (
        <div style={{ background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 8, padding: "10px 12px", marginBottom: 14, fontSize: 12 }}>
          <div style={{ color: "var(--primary-color)", fontWeight: 700, marginBottom: 2 }}>💳 {plan.name}</div>
          <div style={{ color: "var(--text-muted)" }}>Selected payment plan</div>
          <button type="button" onClick={onBack} style={{ background: "none", border: "none", color: "var(--text-muted)", fontSize: 11, padding: 0, marginTop: 4, cursor: "pointer", textDecoration: "underline", fontFamily: "var(--font-body)" }}>Change plan</button>
        </div>
      )}
        ...
        <>
          {commissionAmt && (
            <div className="commission-box">
              <div className="commission-label">Your Commission</div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div className="commission-amt">{fmtExact(commissionAmt, sym)}</div>
                <div style={{ fontSize: 12, color: "var(--text-muted)" }}>{commissionRate}% of price</div>
              </div>
            </div>
          )}
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 8, textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.8px" }}>Your Contact Info</div>
          <input className="sidebar-input" type="text" placeholder="Your Name *"  value={form.name}  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
          <input className="sidebar-input" type="tel"  placeholder="Your Phone *" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} />
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 8, marginTop: 8, textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.8px" }}>Client Details</div>
          <input className="sidebar-input" type="text" placeholder="Client Full Name *" value={form.client_name} onChange={(e) => setForm((f) => ({ ...f, client_name: e.target.value }))} />
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <CountryCodePicker value={form.client_country_code} onChange={(v) => setForm((f) => ({ ...f, client_country_code: v }))} />
            <input style={{ flex: 1, padding: "10px 12px", background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 8, color: "var(--text-dark)", fontSize: 13, fontFamily: "var(--font-body)", outline: "none" }} type="tel" placeholder="Client Phone *" value={form.client_phone} onChange={(e) => setForm((f) => ({ ...f, client_phone: e.target.value.replace(/\D/g, "") }))} />
          </div>
          <input className="sidebar-input" type="email" placeholder="Client Email (optional)" value={form.client_email} onChange={(e) => setForm((f) => ({ ...f, client_email: e.target.value }))} />
          <textarea className="sidebar-input" placeholder="Notes (optional)" rows={2} value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} style={{ resize: "none", marginBottom: 16 }} />
        </>
      ) : (
        <>
          <input className="sidebar-input" type="text" placeholder="Your Name *"    value={form.name}  onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
          <input className="sidebar-input" type="tel"  placeholder="Phone Number *" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))} />
          <textarea className="sidebar-input" placeholder="Message / Notes (optional)" rows={3} value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} style={{ resize: "none", marginBottom: 16 }} />
        </>
      )}
...
const COUNTRY_CODES = [
  ...
  { code: "+966", flag: "🇸🇦", name: "Saudi Arabia" },   { code: "+974", flag: "🇶🇦", name: "Qatar" },
  { code: "+965", flag: "🇰🇼", name: "Kuwait" },         { code: "+973", flag: "🇧🇭", name: "Bahrain" },
  { code: "+968", flag: "🇴🇲", name: "Oman" },           { code: "+1",   flag: "🇺🇸", name: "USA / Canada" },
  { code: "+44",  flag: "🇬🇧", name: "UK" },             { code: "+61",  flag: "🇦🇺", name: "Australia" },
  ...
  { code: "+39",  flag: "🇮🇹", name: "Italy" },          { code: "+34",  flag: "🇪🇸", name: "Spain" },
  { code: "+7",   flag: "🇷🇺", name: "Russia" },         { code: "+55",  flag: "🇧🇷", name: "Brazil" },
  { code: "+92",  flag: "🇵🇰", name: "Pakistan" },       { code: "+880", flag: "🇧🇩", name: "Bangladesh" },
  { code: "+94",  flag: "🇱🇰", name: "Sri Lanka" },      { code: "+977", flag: "🇳🇵", name: "Nepal" },
  { code: "+20",  flag: "🇪🇬", name: "Egypt" },          { code: "+234", flag: "🇳🇬", name: "Nigeria" },
  { code: "+27",  flag: "🇿🇦", name: "South Africa" },   { code: "+254", flag: "🇰🇪", name: "Kenya" },
  { code: "+62",  flag: "🇮🇩", name: "Indonesia" },      { code: "+63",  flag: "🇵🇭", name: "Philippines" },
  { code: "+66",  flag: "🇹🇭", name: "Thailand" },       { code: "+84",  flag: "🇻🇳", name: "Vietnam" },
  { code: "+82",  flag: "🇰🇷", name: "South Korea" },    { code: "+90",  flag: "🇹🇷", name: "Turkey" },
  { code: "+98",  flag: "🇮🇷", name: "Iran" },           { code: "+964", flag: "🇮🇶", name: "Iraq" },
  { code: "+961", flag: "🇱🇧", name: "Lebanon" },        { code: "+962", flag: "🇯🇴", name: "Jordan" },
];
  ...
  return (
    <div style={{ position: "relative" }}>
      <button type="button" onClick={() => { setOpen((o) => !o); setSearch(""); }}
        style={{ display: "flex", alignItems: "center", gap: 6, padding: 10, background: "var(--page-bg)", border: "1.5px solid var(--card-border)", borderRadius: 8, color: "var(--text-dark)", cursor: "pointer", fontSize: 13, whiteSpace: "nowrap", minWidth: 82, fontFamily: "var(--font-body)" }}>
        <span style={{ fontSize: 16 }}>{selected.flag}</span><span>{selected.code}</span><span style={{ fontSize: 10, opacity: 0.5 }}>▼</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 999, background: "var(--card-bg)", border: "1px solid var(--card-border)", borderRadius: 10, overflow: "hidden", minWidth: 220, boxShadow: "0 8px 32px rgba(0,0,0,0.12)" }}>
          <div style={{ padding: "8px 10px", borderBottom: "1px solid var(--card-border)" }}>
            <input autoFocus type="text" placeholder="Search country…" value={search} onChange={(e) => setSearch(e.target.value)}
              style={{ width: "100%", padding: "7px 10px", background: "var(--page-bg)", border: "1px solid var(--card-border)", borderRadius: 6, color: "var(--text-dark)", fontSize: 12, outline: "none", boxSizing: "border-box", fontFamily: "var(--font-body)" }} />
          </div>
          <div style={{ maxHeight: 240, overflowY: "auto" }}>
            {filtered.map((c) => (
              <div key={c.code + c.name} onClick={() => { onChange(c.code); setOpen(false); setSearch(""); }}
                style={{ padding: "9px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", fontSize: 13, background: c.code === value ? "var(--page-bg)" : "transparent", color: c.code === value ? "var(--primary-color)" : "var(--text-dark)", fontWeight: c.code === value ? 700 : 400 }}
                onMouseOver={(e) => (e.currentTarget.style.background = "var(--page-bg)")}
                onMouseOut={(e)  => (e.currentTarget.style.background = c.code === value ? "var(--page-bg)" : "transparent")}>
                <span style={{ fontSize: 16, flexShrink: 0 }}>{c.flag}</span>
                <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.name}</span>
                <span style={{ opacity: 0.45, fontSize: 12, flexShrink: 0 }}>{c.code}</span>
              </div>
            ))}
          </div>
        </div>
